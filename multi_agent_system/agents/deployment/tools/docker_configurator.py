...
class DockerConfiguratorToolOutput(BaseModel):
    """Output schema"""
    dockerfile: str = Field(default="", description="Dockerfile")
    docker_compose: str = Field(default="", description="docker-compose")


class DockerConfiguratorTool(BaseTool):
    """Configure Docker"""
    
    name: str = "docker_configurator"
    description: str = "Perform Configure Docker"
    ...
    def _run(self, project_info: Dict[str, Any], tech_stack: Dict[str, Any], run_manager=None) -> Dict[str, Any]:
        ...
        structured_llm = self.llm.with_structured_output(DockerConfiguratorToolOutput)
