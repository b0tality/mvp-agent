...
3. 每条验收标准要能被一个 HTTP 请求客观判断通过/不通过。
4. 覆盖所有功能需求，尤其是错误处理和边界条件，并且必须覆盖关键不变式：资源标识唯一性/递增（连续创建多个，id 不得重复）、幂等、删除后再次访问返回 404、状态与计数一致性。

关键约束（本流水线只生成后端 API，不生成前端）：
5. 每条验收标准必须能映射为「HTTP 请求 + 响应断言」（状态码 / 响应体字段 / 列表长度 / 数据一致性）。
6. 禁止出现纯 UI 行为：点击按钮、勾选复选框、颜色标签、页面跳转、前端通知等。
   若原始需求是 UI 行为，改写为对应的 API 行为。例如：「点击添加按钮后列表出现该待办」→「POST /todos 返回 201，响应体含新待办，且 GET /todos 列表长度 +1」。
7. 不要臆造用户没提的功能（标签/优先级/提醒/通知等），只验收用户明确要求或为实现其意图所必需的行为。
8. FastAPI 对必填字段缺失/类型错误默认返回 422（不是 400）。涉及「缺失/非法输入」的验收标准要写 422，别想当然写 400；若要 400 必须说明「注册 exception_handler 转换」。"""
